...
def apply_filters(
    ...
    jd_text: str | None,
    config: FilterConfig,
) -> tuple[bool, list[FilterVerdict], Seniority, int | None]:
    """Run all filter stages. Returns (passed, verdicts, seniority, experience_years)."""
    ...
    verdicts.append(v)
    if not v.passed:
        return False, verdicts, Seniority.unknown, None
    ...
    verdicts.append(v)
    if not v.passed:
        return False, verdicts, seniority, None

    # 5. Experience
    v, years = _check_experience(jd_text, config.max_experience_years)
    verdicts.append(v)
    if not v.passed:
        return False, verdicts, seniority, years
    ...
    return True, verdicts, seniority, years
