...
_BOARD_PATTERNS: dict[str, str] = {
    ...
    "lever": r"/[^/]+/[0-9a-f-]{36}$",
    "ashby": r"/[^/]+/[0-9a-f-]{36}$",
    "simplyhired": r"/job/[A-Za-z0-9_-]+",
    "linkedin": r"/jobs/view/[\w%-]+-\d+",
}
...
_BOARD_MAP = {
    ...
    "simplyhired": JobBoard.simplyhired,
...
async def _crawl_targets(config: ScraperConfig) -> list[SearchResult]:
    ...
    async with AsyncWebCrawler() as crawler:
        for i, target in enumerate(crawl_cfg.targets):
            ...
            target_host = _normalize_host(target.url)
            ...
            try:
                ...
                for link in result.links.get("internal", []) + result.links.get("external", []):
                    ...
                    url = canonicalize_job_url(urljoin(target.url, href))
                    link_host = _normalize_host(url)

                    # Keep each crawl target scoped to its board host family.
                    if target_host in _GREENHOUSE_HOSTS:
                        if link_host not in _GREENHOUSE_HOSTS:
                            continue
                    elif link_host != target_host:
                        continue
            ...
            if i < len(crawl_cfg.targets) - 1:
                await asyncio.sleep(crawl_cfg.request_delay)
